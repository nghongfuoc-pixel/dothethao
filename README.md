...
Lưu ý: schema **không** được quản lý qua Supabase CLI (`supabase/migrations`) — repo không có thư mục cấu hình CLI, nên dashboard hiện "No migrations"/"No repository connected" là bình thường, không phải lỗi. Dữ liệu `categories`/`products` hiện tại trên Supabase khớp đúng với [database/supabase_schema.sql](database/supabase_schema.sql) (đã đối chiếu qua REST API). File này dùng để tái tạo schema khi tạo project mới, nhưng **chưa chắc phản ánh đúng 100%** cấu trúc/RLS hiện tại nếu có ai từng chỉnh trực tiếp trên Supabase (Table Editor, SQL Editor, v.v.) mà quên cập nhật lại file — nên coi đây là bản gần đúng nhất, cần đối chiếu lại trước khi dùng để tái tạo production.
